...
export const registerUser = async (req, res) => {
    ...
    const hashedPassword = await bcsrypt.hash(password, 10);
...
export const loginUser = async (req, res) => {
    ...
    const isPasswordValid = await bcsrypt.compare(password, users.password);
    if (!isPasswordValid) {
        return res.status(401).json({ message: 'Invalid credentials' });
    }
    const token = jwt.sign({ id: users.id }, process.env.KEY, { expiresIn: '1h' });
